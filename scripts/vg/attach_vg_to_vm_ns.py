...
def get_vm_vg_list(CLUS_LIST):
  ...
  for i in range(START, END):
    #vg_name_a = "vdi-vm-{0}-vg-1".format(i)
    #vg_name_b = "vdi-vm-{0}-vg-2".format(i)
    #vg_pre = "DB-VG-"
    cluster_name = CLUS_LIST[0] 
    tmp = dict()
    tmp["vm_name"] = "ST_PC-A-PE-1_vdb-vdi_dp-cmp-dl_centos7_scsi_" + str(i).rjust(4, '0')
    tmp["cluster_name"] = cluster_name
    tmp["vg_list"] = list()
    tmp["vg_list"].append({"attach_type": "iscsi", "vg_name": "vdi-vm-{0}-vg-1".format(i)})
    if i > 80:
      tmp["vg_list"].append({"attach_type": "iscsi", "vg_name": "vdi-vm-{0}-vg-2".format(i)})
